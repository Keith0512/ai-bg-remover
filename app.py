...
def upscale_image(image, scale_factor=2):
    """使用 Lanczos 演算法進行高品質放大，讓下載的圖片接近 2K/4K 質感"""
...
# --- 核心功能：驗證 API Key (使用最簡單的 requests.post) ---
...
# --- 分析函式 (v1.8: 固定 4 風格 + 強制高畫質 Prompt) ---
def analyze_image_with_gemini(api_key, image, model_name):
    ...
    def _send_request(target_model):
        # 使用最單純的字串串接，避開 Session Adapter 問題
        url = f"https://generativelanguage.googleapis.com/v1beta/models/{target_model}:generateContent?key={api_key}"
        res = None
        last_error = None
        for i in range(3):
            try:
                # 簡單的 requests.post，timeout 設定為 60s
...
def generate_image_with_gemini(api_key, product_image, base_prompt, model_name, user_extra_prompt="", ref_image=None):
    ...
    def _send_request(target):
        ...
        for i in range(3):
            try:
                # 簡單的 requests.post，timeout 設定為 90s (生圖比較久)
...
with st.sidebar:
    ...
    st.divider()
    st.caption("v1.8 (Stable Restored)")
